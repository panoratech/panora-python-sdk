...
class HrisEmployerbenefit:
    ...
    def add_employer_benefits(self, connection_token: str, x_connection_token: str, request_body: List[components.UnifiedEmployerBenefitInput], remote_data: Optional[bool] = None) -> operations.AddEmployerBenefitsResponse:
        r"""Add a batch of EmployerBenefits"""
        hook_ctx = HookContext(operation_id='addEmployerBenefits', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        url = base_url + '/hris/employerbenefit/batch'
        ...
        headers = { **utils.get_headers(request), **headers }
        req_content_type, data, form = utils.serialize_request_body(request, operations.AddEmployerBenefitsRequest, "request_body", False, False, 'json')
        if req_content_type is not None and req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        if data is None and form is None:
            raise Exception('request body is required')
        ...
        res = operations.AddEmployerBenefitsResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
        
        if http_res.status_code == 200:
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[operations.AddEmployerBenefitsResponseBody])
                res.object = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
            ...
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[List[components.UnifiedEmployerBenefitOutput]])
                res.unified_employer_benefit_outputs = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
        elif http_res.status_code >= 400 and http_res.status_code < 500 or http_res.status_code >= 500 and http_res.status_code < 600:
